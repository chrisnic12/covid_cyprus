...
%%% This script considers incidents from local transmission only
...
incidence_real = incidence_real.Incidence_local;
incidence_real = incidence_real(7:end-2);
...
%%% reporting rates.
for j=2:num_periods
        [state_vec(:,:,j),alpha_vec(j,:), beta_vec(j,:),incidence_vec(:,:,j), Re_vec(j,:)]=MCMC2_local(alpha_vec(j-1,:),state_vec(:,:,j-1),incidence_vec(:,:,j-1),max(2,(j+1)/2),2,1,1,(j-1)*num_times+1,(j)*num_times,(j-2)*num_times+1,j);
end
