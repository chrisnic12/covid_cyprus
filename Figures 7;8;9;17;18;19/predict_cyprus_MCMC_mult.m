...
%%% This function is almost identical to predict_cyprus_MCMC.m. The only
%%% difference is in the for loop the range of t, which is suitable for
%%% using in MCMC2 where we need to compute delays for days in later
%%% periods.

%%% This function requires initial values S0, E0, IS0, IA0, y0, num_times,
%%% init_incidence and init_day.
...
function [incidence_synthetic,state]=predict_cyprus_MCMC_mult(num_times,init_incidence,alpha,beta,S0,E0,Is0,Ia0,y0,init_day)
...
for t=init_day:(init_day+num_times-1)
    x=SEIR_cyprus(x);
...
state=x_mat(1:4,num_times+init_day-1);
